...
export const useJobStore = defineStore('jobs', () => {
  ...
  const jobs = ref([
    ...
    },
    {
      ...
      type: 'Full-time',
      category: 'การตลาด',
      description: 'รับสมัคร Digital Marketing Manager ที่มีประสบการณ์...',
      requirements: [
        'ปริญญาตรี การตลาด/สื่อสารมวลชน',
        'มีประสบการณ์ 3-5 ปี',
        'เชี่ยวชาญ SEO, SEM, Social Media'
      ],
      benefits: [
        'ประกันสังคม',
        'ค่าคอมมิชชั่น',
  ...
  const searchJobs = (query) => {
    ...
    return activeJobs.value.filter(job => {
      const matchPosition = !query.position || 
        job.title.toLowerCase().includes(query.position.toLowerCase())
      
      const matchLocation = !query.location || 
        job.location.toLowerCase().includes(query.location.toLowerCase())
      
      const matchCompany = !query.company || 
